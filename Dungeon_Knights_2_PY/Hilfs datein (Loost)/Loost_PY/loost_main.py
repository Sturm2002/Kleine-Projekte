...
3/10 - Optimize Potion System
1/10 - Inventory Duplicates
4/10 - New Equip anti delete

2/10 - Waypoints
3/10 - Death (Maybe Revive-Potion ?)

2/10 - More Enemys
3/10 - Moving Enemys ?
7/10 - Intense Enemy Fights

6/10 - XP / lvl System / Balancing
9/10 - Story ?
8/10 - Trading / Coins
...
Regen potions over Max Health
when only 1 enemy no select
Input Exceptions
...
V0.4 / 15.12.2020
-Help Menu
-Show Items after Open Chest       <-------- nice
...
LoostHelp = """
    ...
    """+colorama.Fore.LIGHTBLACK_EX+"""Command   |  Synonyms   | Description"""+colorama.Fore.WHITE+"""
...
Items = [
    ["Swords",[# Name[0],Cat.[1],DmgMult[2],PenMult[3],Rarity[4]
        [colorama.Fore.WHITE+"Wooden Sword"+colorama.Fore.WHITE,"Swrd",0.2,0.2,1],
        [colorama.Fore.WHITE+"Stone Sword"+colorama.Fore.WHITE,"Swrd",0.3,0.3,1],
        [colorama.Fore.WHITE+"Cooper Sword"+colorama.Fore.WHITE,"Swrd",0.35,0.35,1],
        [colorama.Fore.LIGHTGREEN_EX+"Iron Sword"+colorama.Fore.WHITE,"Swrd",0.45,0.45,2],
        [colorama.Fore.LIGHTGREEN_EX+"Silver Sword"+colorama.Fore.WHITE,"Swrd",0.5,0.5,2],
    ...
    ["Shields",[# Name[0],Cat.[1],DefMult[2],Rarity[3]
        [colorama.Fore.WHITE+"Wooden Shield"+colorama.Fore.WHITE,"Shld",0.3,1],
        [colorama.Fore.LIGHTGREEN_EX+"Reinforced Wooden Shield"+colorama.Fore.WHITE,"Shld",0.5,2],
        [colorama.Fore.CYAN+"Iron Shield"+colorama.Fore.WHITE,"Shld",0.7,3],
        [colorama.Fore.MAGENTA+"Reinforced Iron Shield"+colorama.Fore.WHITE,"Shld",0.9,4],
        [colorama.Fore.LIGHTRED_EX+"Shield Of the Black Night"+colorama.Fore.WHITE,"Shld",1.1,5]
        ]],
...
User1 = Player(0,0,100,20)
User1.Xpos = 4

User2 = Player(0,0,100,20)
User2.Xpos = 3
...
class Player():
    def __init__(self,StartPosX=0,StartPosY=0,Health=100,StandDmg=10,StandDefense=20,ArmorPen=1):
        ...
        self.Xp = 0
        self.StandXp = 10
        self.RequiredXp = self.StandXp * (1/self.Level)
        ...
        self.GenedFields = [[0,0]]
        if self.OptCheatMode == True:
            self.Inventory = [[colorama.Fore.WHITE+"Wooden Sword"+colorama.Fore.WHITE,"Swrd",0.2,0.2,1],
        [colorama.Fore.WHITE+"Stone Sword"+colorama.Fore.WHITE,"Swrd",0.3,0.3,1],
        [colorama.Fore.WHITE+"Cooper Sword"+colorama.Fore.WHITE,"Swrd",0.35,0.35,1],
        [colorama.Fore.LIGHTGREEN_EX+"Iron Sword"+colorama.Fore.WHITE,"Swrd",0.45,0.45,2],
        [colorama.Fore.LIGHTGREEN_EX+"Silver Sword"+colorama.Fore.WHITE,"Swrd",0.5,0.5,2],
                ...
                if enemCounter == int(inp):
                    ...
                    if enem.Health <= 0:
                        ...
                        if len(enem.Inventory) <= 0 and enem.AmountKeys <= 0:
                            ...
                            self.Xp += enem.Xp
                            Outstr += str(enem.Name) + " Got " + str((self.StandDmg * self.DmgMultiplier-(enem.Defense-(self.StandArmorPen*self.ArmorPenMult)))) + " Dmg! And is Dead!\n"
                        ...
                        Outstr += str(enem.Name) + " Got " + str((self.StandDmg * self.DmgMultiplier-(enem.Defense-(self.StandArmorPen*self.ArmorPenMult)))) + " Dmg! And has Now " + str(enem.Health) + " Hp\n"
    ...
    def UseItem(self):
        ...
        if self.Inventory[int(ItemToUse)][1] == "Swrd":
            self.DmgMultiplier = 1
            self.ArmorPenMult = 1
            self.DmgMultiplier += float(self.Inventory[int(ItemToUse)][2])
            self.ArmorPenMult += float(self.Inventory[int(ItemToUse)][3])
            ...
            Outstr += str(self.Inventory[int(ItemToUse)][0])
            #sleep(2)
            #print(str(self.Inventory[int(ItemToUse)][0])+str(self.Inventory[int(ItemToUse)][1])+str(self.Inventory[int(ItemToUse)][2])+str(self.Inventory[int(ItemToUse)][3]))
            del self.Inventory[int(ItemToUse)]

        elif self.Inventory[int(ItemToUse)][1] == "Misc":
            if self.Inventory[int(ItemToUse)][2] == "Flashlight":
                if self.MapRange >= MapRangeMax:
        ...
        for potn in self.PotionEffects:
            if potn[4] <= 0:
                ...
                if potn[2] == "Regen":
                    ...
                    self.ArmorPenMult += potn[3]

    def CheckXp(self):
        global LvlProcess
        if self.Xp >= self.RequiredXp:
            self.Level += 1
            self.StandXp += self.RequiredXp
            self.Xp = 0
            self.AmountKeys +=1 
            self.MaxHealth += 2
            self.StandDmg += 1
        LengthProc = 10
        LvlProcess = "|" + int(((self.Xp / self.RequiredXp) * LengthProc)) * "#" + (LengthProc -int(((self.Xp / self.RequiredXp) * LengthProc)))* "/" + "|"
        return LvlProcess
...
class Enemy():
    def __init__(self,StartXpos,StartYpos,Health,Dmg,Name,StandArmorPen=0,Standdefense=5,GiveXp =5):
        self.Name = Name
        self.Xpos = StartXpos
        self.Ypos = StartYpos
        self.Xp = GiveXp
...
def ChoosePlayerClass():
    ...
    if inp == "Assassin" or inp == "1":
        User = Player(Health=60,StandDmg=30,StandDefense=5,ArmorPen=10)
    elif inp == "Attacker" or inp == "2": 
        User = Player(Health=70,StandDmg=25,StandDefense=10,ArmorPen=5)
...
def GenOnCor(Xpos,Ypos):
    global MapGenValueMax,MapGenValueMin,EnemysArr,ChestsArr,User,Enemy
    ...
    for field in User.GenedFields:
        ...
        for i in range(val):
            ...
            if randnum == 1:
                ...
                randenem = randint(1,3)

                if randenem == 1:
                    NewZombie = Enemy(Xpos,Ypos,50,10,(colorama.Fore.GREEN +"Zombie"+colorama.Fore.WHITE),Standdefense=7)
                    EnemysArr.append(NewZombie)

                elif randenem == 2:
                    NewSkell = Enemy(Xpos,Ypos,35,15,(colorama.Fore.LIGHTBLACK_EX+"Skeleton"+colorama.Fore.WHITE),Standdefense=3)
                    EnemysArr.append(NewSkell)
                            
                elif randenem == 3: 
                    NewOrk = Enemy(Xpos,Ypos,75,5,(colorama.Fore.LIGHTGREEN_EX+"Ork"+colorama.Fore.WHITE),Standdefense=10)
                    EnemysArr.append(NewOrk)
    ...
    for y in range(0,User.MapRange*2+1):
        ...
        for x in range(0,User.MapRange*2+1):
            ...
            if y == User.MapRange and x == User.MapRange:
                MMap[y][x] = colorama.Fore.GREEN + "#" + colorama.Fore.WHITE
                ...
                for field in User.GenedFields:
                    ...
                    for i in range(val):
                        ...
                        if randnum == 1:
                            ...
                            randenem = randint(1,3)
                            if randenem == 1:
                                NewZombie = Enemy(x,NewY,40,10,(colorama.Fore.GREEN +"Zombie"+colorama.Fore.WHITE),Standdefense=7,GiveXp=5)
                                EnemysArr.append(NewZombie)
                            elif randint == 2:
                                NewSkell = Enemy(x,NewY,20,15,(colorama.Fore.LIGHTBLACK_EX+"Skeleton"+colorama.Fore.WHITE),Standdefense=3,GiveXp=7)
                                EnemysArr.append(NewSkell)
                            
                            elif randint == 3: 
                                NewOrk = Enemy(x,NewY,60,5,(colorama.Fore.LIGHTGREEN_EX+"Ork"+colorama.Fore.WHITE),Standdefense=10,GiveXp=3)
                                EnemysArr.append(NewOrk)
                            #User.GenedFields.append([x,NewY])
                            val -= 1
                        if val <= 0:
...
def LoadGame():
    ...
    if FileCheck.exists():
        ...
        User.Xp = savejson['PXp']
        User.StandXp = savejson['PStandXp']
        User.RequiredXp = savejson['PReqXp']
...
def SaveGame():
    ...
    JsonStr = {
        ...
        "PXp" : User.Xp,
        "PStandXp":User.StandXp,
        "PReqXp" : User.RequiredXp,
    ...
    for enem in EnemysArr:
        JsonStr["Enemys"] += [[enem.Name,enem.Xpos,enem.Ypos,enem.Health,enem.Defense,enem.Dmg,enem.ArmorPen,enem.alive,enem.AmountKeys,enem.AmountCoins,enem.Inventory,enem.Xp]]
...
def Main():
    ...
    while running:
        ...
        printstr(f"""
    Status :     <<<Level {colorama.Fore.YELLOW + str(User.Level) + colorama.Fore.WHITE}>>>
                 {User.CheckXp()}
                {colorama.Fore.RED+str(int(User.Health))+colorama.Fore.WHITE} Hp | {User.Xpos}x / {User.Ypos}y
                {colorama.Fore.LIGHTRED_EX+ str(int(User.StandDmg*User.DmgMultiplier))+colorama.Fore.WHITE}+{colorama.Fore.MAGENTA+str(int(User.StandArmorPen*User.ArmorPenMult))+colorama.Fore.WHITE} Dmg | {colorama.Fore.LIGHTBLUE_EX+str(int(User.StandDefense * User.DefenseMultShield))+colorama.Fore.WHITE} Def
            \n\n"""+
    str(ShowMap())
